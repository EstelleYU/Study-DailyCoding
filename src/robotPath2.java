/* -------------------------------
45_robotPath2 (Advanced)
Monday, August 1st, 2022
Daily Coding from CodeStates
------------------------------------
With a matrix (MxN), element 1 means obstacle and element 0 means a movable path.
The Robot can select moving K squares or rotate 90 degrees at once.
The robot's current position, direction, the target point,
and the target direction of the target point are given as input data.
Upper side is 1, right side is 2, lower side is 3, and left is 4.
Return the count of all movements until robot reaches the target point and direction.
------------------------------------*/
...
public class robotPath2 {
    public static void main(String[] args) {
        ...
        int[] dst = new int[]{2, 2};
        int dDir = 2;
        int output = robotPathf(room, src, sDir, dst, dDir);
        System.out.println(output);   // --> 11
    }
    public static int robotPathf(int[][] room, int[] src, int sDir, int[] dst, int dDir) {
        // 가로와 세로의 길이
        int R = room.length;
        int C = room[0].length;
