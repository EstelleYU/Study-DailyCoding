...
레퍼런스는 모두 주어집니다. 구간의 최대값, 합도 동일한 로직으로 구현하면 됩니다.
------------------------------ */
...
public class rangeMinimum{
    public static void main(String[] args) {
        ...
        int[][] ranges = new int[][]{ {1, 4}, {0, 3}};
        int[] mins = rangeMinimum_f(arr, ranges);
        System.out.println(Arrays.toString(mins)); // --> [2, 1]

        arr = new int[]{10,11,12,3,6,7,8,9};
        ranges = new int[][]{ {4,7}, {0, 2}};
        mins = rangeMinimum_f(arr, ranges);
        System.out.println(Arrays.toString(mins)); // --> [6, 10]

        arr = new int[]{1,3,2,7,9,11};
        ranges = new int[][]{ {1,4}, {0, 3}};
        mins = rangeMinimum_f(arr, ranges);
        System.out.println(Arrays.toString(mins)); // --> [2, 1]
        ...
        ranges = new int[][]{ {1,1}};
        mins = rangeMinimum_f(arr, ranges);
        System.out.println(Arrays.toString(mins)); // --> [2]

    }
    //createMinTree 메서드 활용을 위해 필드변수 선언
    static int[] tree;

    public static int[] rangeMinimum_f(int[] arr, int[][] ranges) {
